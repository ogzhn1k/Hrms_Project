package oguzhanspring.hrms.hrms.core.utilities.dtoConverter.concretes;
...
public class DtoConverterManager implements DtoConverterService{
	...
	public <S, T> List<T> entityToDto(List<S> listTypeEntityClass, Class<T> targetDtoClass) {
		...
		return listTypeEntityClass.stream().map(element -> this.modelMapper.map(element, targetDtoClass)).collect(Collectors.toList());
	}

	@Override
	public <T> Object dtoToEntity(Object dtoClass, Class<T> targetEntityClass) {
		return this.modelMapper.map(dtoClass, targetEntityClass);
	}
